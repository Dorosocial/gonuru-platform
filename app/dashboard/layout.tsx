...
const icons = {
  ...
  subscriptions: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="17 1 21 5 17 9"/><path d="M3 11V9a4 4 0 0 1 4-4h14"/><polyline points="7 23 3 19 7 15"/><path d="M21 13v2a4 4 0 0 1-4 4H3"/></svg>,
  gift: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M20 12v10H4V12"/><path d="M22 7H2v5h20V7z"/><path d="M12 22V7"/><path d="M12 7H7.5a2.5 2.5 0 0 1 0-5C11 2 12 7 12 7z"/><path d="M12 7h4.5a2.5 2.5 0 0 0 0-5C13 2 12 7 12 7z"/></svg>,
  ...
  payouts: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="1" y="4" width="22" height="16" rx="2" ry="2"/><line x1="1" y1="10" x2="23" y2="10"/></svg>,
...
const navItems = [
  ...
  { href: '/dashboard/show-love', label: 'Send Me a Gift', icon: icons.gift },
  { href: '/dashboard/bookings', label: 'Bookings', icon: icons.bookings },
  { href: '/dashboard/payouts', label: 'Payouts', icon: icons.payouts },
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-black text-white flex">
      <aside className="hidden md:flex flex-col w-56 border-r border-zinc-800 min-h-screen fixed left-0 top-0">
          ...
          {navItems.map(item => (
            <Link
              ...
              style={isActive(item.href)
                ? { backgroundColor: G, color: '#000', fontWeight: 700 }
                : { color: '#71717a' }
              }
            >
              {item.icon}
        ...
        <div className="p-4 border-t border-zinc-800">
          <button
            ...
            className="flex items-center gap-3 text-zinc-500 text-sm hover:text-white transition w-full"
          >
            {icons.signout}
            Sign out
          </button>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-black border-t border-zinc-800 flex justify-around py-2 z-50">
        {navItems.slice(0, 5).map(item => (
          <Link
            key={item.href}
            href={item.href}
            className="flex flex-col items-center gap-0.5 px-2 py-1 text-xs transition-all"
            style={isActive(item.href) ? { color: G } : { color: '#71717a' }}
          >
            {item.icon}
